...
{
    [Serializable]
    public class LocalTranscriptionEvent : UnityEvent<string> { }
    ...
    public class STTHandler : MonoBehaviour
    {
        ...
        public LocalTranscriptionEvent onFinalResult = new LocalTranscriptionEvent();
        public LocalTranscriptionEvent onInterimResult = new LocalTranscriptionEvent();
        ...
        private const string CredentialFileName = "gcp_credentials";
        ...
        public bool isReadyToTranscribe = false;
        ...
        private void Awake()
        {
            ...
            //string credentialsPath = Path.Combine(Application.temporaryCachePath, CredentialFileName);
            //string credentialsPath = "file://" + Path.Combine(Application.streamingAssetsPath, CredentialFileName);

           
            string dbPath = "";
                // Android
                string oriPath = Path.Combine(Application.streamingAssetsPath, CredentialFileName);
            ...
            //Environment.SetEnvironmentVariable("GOOGLE_APPLICATION_CREDENTIALS", dbPath);
        ...
        private void InitializeMicrophoneAndBeginStream()
        {
            ...
            //AudioConfiguration audioConfiguration = AudioSettings.GetConfiguration();
            _audioSource.clip = Microphone.Start(_microphoneName, true, 20, 16000);
        ...
        public float _delayBeforeSilence = 2;
        public float _delayCountdown = 0;
        ...
        void Update()
        {
            if(!_isSilent)
            {
                if (_delayCountdown < _delayBeforeSilence)
                {
                ...
                else
                {
                    ...
                    Debug.Log("onFinalResult");
                    StartCoroutine(OnSilent());
                }
            }
        }

        IEnumerator OnSilent()
        {
            GetSTT.GetComponent<GetSpeechToText>().isFinalResult = true;
            GetSTT.GetComponent<GetSpeechToText>().finalResult = transcript;
            if (enableDebugLogging)
            {
            ...
            //Microphone.End(_microphoneName);
            if (isReadyToTranscribe) // truong hop dang thu am va nguoi dung ket thuc cau va khong bam nut dung
            {
                dataBase64 = SavWav.Save("demoIM", _audioSource.clip);
              //  Debug.LogError(dataBase64);
            }
            //sSave = true;
        ...
        private async Task HandleTranscriptionResponses()
        {
            while (await _streamingCall.ResponseStream.MoveNext(default))
            {
                ...
                Debug.Log("HandleTranscriptionResponses " + transcript + "  result.IsFinal   " + result.IsFinal);
                ...
                {
                    GetSTT.GetComponent<GetSpeechToText>().isFinalResult = false;
                    if (returnInterimResults)
                    {
                        GetSTT.GetComponent<GetSpeechToText>().outputInterim = transcript;
                        if (enableDebugLogging)
                        {
               ...
               /* if (isSave) // kiem tra file da save chua neu save thi bat lai microphone
                {
                    isSave = false;
                    AudioConfiguration audioConfiguration = AudioSettings.GetConfiguration();
        ...
        {
            GCP_CREDENTIAL credential = new GCP_CREDENTIAL();

            string json = JsonUtility.ToJson(credential);
